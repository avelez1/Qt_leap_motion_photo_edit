...
void QtLeapHand::update(Leap::Hand *hand)
{
    ...
    this->setSphereCenter(QVector3D(hand->sphereCenter().x,
                                    hand->sphereCenter().y,
                                    hand->sphereCenter().z));
    this->setPinchStrength(hand->pinchStrength());
    this->setDirection(QVector3D(hand->direction().x,
                                 hand->direction().y,
                                 hand->direction().x));
    this->setPosition(QVector3D(hand->palmPosition().x,
                                 hand->palmPosition().y,
                                 hand->palmPosition().x));
...
float QtLeapHand::getPinchStrength() const
{
    Q_D(const QtLeapHand);
    return d->m_pinchStrength;
}
...
void QtLeapHand::setPinchStrength(float pinchStrength)
{
    Q_D(QtLeapHand);
    if (d->m_pinchStrength != pinchStrength)
    {
        d->m_pinchStrength = pinchStrength;
        emit pinchStrengthChanged();
    }
}
