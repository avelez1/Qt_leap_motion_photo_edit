...
float Image22::smartAccessor(int x, int y, int z, bool clamp) const{
    // // --------- HANDOUT  PS02 ------------------------------
...
long long Image22::number_of_elements()const {
    // // --------- HANDOUT  PS01 ------------------------------
...
const float & Image22::operator()(int x) const {
    // // --------- HANDOUT  PS01 ------------------------------
...
const float & Image22::operator()(int x, int y) const {
    // // --------- HANDOUT  PS01 ------------------------------
...
const float & Image22::operator()(int x, int y, int z) const {
    // // --------- HANDOUT  PS01 ------------------------------
...
float & Image22::operator()(int x, int y) {
    // // --------- HANDOUT  PS01 ------------------------------
...
float & Image22::operator()(int x, int y, int z) {
    // // --------- HANDOUT  PS01 ------------------------------
...
void Image22::set_color(float r, float g, float b) {
    // --------- HANDOUT  PS01 ------------------------------
...
void Image22::create_rectangle(int xstart, int ystart, int xend, int yend,
...
void Image22::create_line(int xstart, int ystart, int xend, int yend,
                        float r, float g, float b) {
    // --------- HANDOUT  PS01 ------------------------------
...
int Image22::debugWriteNumber = 0;

Image22::Image22(int x, int y, int z, const std::string &name_) {
...
void Image22::initialize_image_metadata(int x, int y, int z,  const std::string &name_) {
    dim_values[0] = 0;
    dim_values[1] = 0;
    dim_values[2] = 0;
    stride_[0] = 0;
...
Image22::Image22(const std::string & filename) {
...
Image22::~Image22() { } // Nothing to clean up

void Image22::write(const std::string &filename) const {
...
void Image22::debug_write() const {
...
float Image22::uint8_to_float(const unsigned char &in) {
    return ((float) in)/(255.0f);
}

unsigned char Image22::float_to_uint8(const float &in) {
    float out = in;
    if (out < 0)
        out = 0;
    if (out > 1)
...
void compareDimensions(const Image22 & im1, const Image22 & im2)  {
...
Image22 operator+ (const Image22 & im1, const Image22 & im2) {
    compareDimensions(im1, im2);
    long long total_pixels = im1.number_of_elements();

    Image22 output(im1.extent(0), im1.extent(1), im1.extent(2));
...
Image22 operator+ (const Image22 & im1, const float & c) {
    long long total_pixels = im1.number_of_elements();
    Image22 output(im1.extent(0), im1.extent(1), im1.extent(2));
    for (int i = 0 ; i < total_pixels; i++) {
        output(i) = im1(i) + c;
...
Image22 operator+(const float & c, const Image22 & im1) {
    long long total_pixels = im1.number_of_elements();
    Image22 output(im1.extent(0), im1.extent(1), im1.extent(2));
    for (int i = 0 ; i < total_pixels; i++) {
        output(i) = im1(i) + c;
